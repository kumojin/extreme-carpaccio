...
export const listSellers = (sellerService: SellerService) => async (_: Request, response: Response) => {
...
export const sellersHistory = (sellerService: SellerService) => async (request: Request, response: Response) => {
  const { error, value } = sellersHistoryValidator.validate(request.query.chunk);
...
export const registerSeller = (sellerService: SellerService) => async (request: Request, response: Response) => {
  const { error, value } = registerSellerValidator.validate(request.body);
  if (error) {
    response.status(StatusCodes.BAD_REQUEST).send({ message: messageFromValidationError(error) });
    return;
  }

  const { name: sellerName, url: sellerUrl, password: sellerPwd } = value;
  const isAuthorized = await sellerService.isAuthorized(sellerName, sellerPwd);
  if (!isAuthorized) {
    response.status(StatusCodes.UNAUTHORIZED).send({ message: 'invalid name or password' });
    return;
  }
